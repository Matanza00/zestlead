// src/components/dashboard/RecentLeads.js
'use client';
      ...
      <table className="w-full">
        <thead>
          <tr>
            ...
            <th className="text-left font-semibold text-black text-base">Phone/Email</th>
            <th className="text-left font-semibold text-black text-base">Property / Area</th>
            <th className="text-left font-semibold text-black text-base">Status</th>
            <th className="text-right font-semibold text-black text-base">
        ...
        <tbody>
          {items.map((it, idx) => {
            const area = it?.meta?.area ?? '—';
            const price = it?.meta?.price ?? '—';
            return (
              <tr key={it.id ?? idx} className="border-b border-gray-200/50">
                <td className="px-0 p-4 font-medium text-base text-neutral whitespace-nowrap">{it.name}</td>
                <td className="px-0 p-4 text-base font-normal text-neutral whitespace-nowrap">{it.contact ?? '—'}</td>
                <td className="px-0 p-4 text-base font-normal text-neutral whitespace-nowrap">
                  {area}{price ? ` • ${price}` : ''}
                </td>
                <td className="px-0 p-4 font-medium text-base text-neutral whitespace-nowrap">{it.status}</td>
